...
<script>
	...
	export default{
		...
		methods:{
			...
			loadMore() {
				...
				if(this.currentPage>this.totalPages){
			  	...
			  	axios.get(`/appapi/silo/eventForH5?categoryId=men&pageIndex=${this.currentPage}&timestamp=1542975837584&summary=e80f9e4dd73b598dd3a861dbe846e857&platform_code=H5`).then(res=>{
		...
		mounted(){
			axios.get("/appapi/home/mktBannerApp/v3?silo_id=2013000100000000002&platform_code=PLATEFORM_H5")
			.then(res=>{
			...
			axios.get("/appapi/cms/cmsDetail/v3?silo=2013000100000000002&ids=2121000100000000234&timestamp=1542975838194&summary=d6bc99771aac453e1eb4f812b836e964&platform_code=H5").then(res=>{
				this.lilist = res.data.resultList[0].data;
			}).catch(err=>{
				console.log('请求失败');
			})

			axios.get("/appapi/silo/eventForH5?categoryId=men&pageIndex=1&timestamp=1542975837584&summary=e80f9e4dd73b598dd3a861dbe846e857&platform_code=H5").then(res=>{
